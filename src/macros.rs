...
///
/// translates to:
///       HListPrependCodec(&uint8, HListPrependCodec(&uint8, &hnil_codec))
///
/// which evaluates to:
...
macro_rules! hcodec {
    ...
    { $head:expr } => {
        HListPrependCodec($head, &hnil_codec)
    };
    { $head:expr, $($tail:expr),+ } => {
        HListPrependCodec($head, &(hcodec!($($tail),+)))
    };
    // { $($head:expr),+ : $tail:expr } => {
...
//     };
//     { { $tail:expr } $head:expr } => {
//         HListPrependCodec($head, $tail)
//     };
//     { { $tail:tt } $head:expr, $($rest:tt),+ } => {
//         HListPrependCodec($head, hcodec_expr_tail!({ $tail } $($rest),+))
//     };
// }
